...
try {
  ...
  run('/usr/bin/openssl', ['pkcs12', '-in', p12, '-clcerts', '-nokeys', '-out', cert, '-passin', 'env:MACOS_CERTIFICATE_PASSWORD']);
  const actual = run('/usr/bin/openssl', ['x509', '-in', cert, '-noout', '-fingerprint', '-sha1']).split('=').at(-1).replace(/[^a-f0-9]/gi, '').toUpperCase();
  ...
  run('/usr/bin/security', ['import', p12, '-k', keychain, '-P', process.env.MACOS_CERTIFICATE_PASSWORD, '-T', '/usr/bin/codesign']);
