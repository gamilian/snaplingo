...
macOS 使用固定自签名证书，未经 Apple 公证。首次启动被拦截时，请在“系统设置 → 隐私与安全性”中选择“仍要打开”。截图需要屏幕录制权限；选中文本和界面元素检测需要辅助功能权限。可以稍后授权，先使用文本翻译和文件 OCR。
...
更新前退出 SnapLingo，覆盖安装同一架构版本。固定自签名证书保持不变时，屏幕录制授权应跨版本保留；仍请在发布验收中实际检查。正常覆盖安装不应删除 API 配置或历史记录。
