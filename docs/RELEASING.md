...
- 授权弹窗只在系统检测成功且当前应用缺少屏幕录制权限时出现，说明当前状态、权限用途，以及重装/更新后旧授权可能未生效的原因，提供“去授权”和“稍后”；返回后自动检测并关闭，点击授权前再次检测。已授权、检测中或检测失败时不弹授权框，重新打开设置时等待新检测结果，避免沿用上次的未授权状态。辅助功能在使用相关功能时再请求，不在首次弹窗展示。引导只出现在设置窗口，不会在翻译、文件 OCR、贴图窗口重复弹出。
- **设置 → 通用 → 系统权限** 显示当前应用的实时授权状态；检测失败显示“检测失败”，不当作“待授权”。只对确认缺少的权限显示授权及重置入口。“授权遇到问题？”中提供重新检测和按需重启；首次弹窗尝试授权后，也可从同名链接进入此处。当前使用的系统检测接口只返回当前进程是否有权限，不能区分尚未授权、用户撤销或旧身份授权未生效；文案以“可能”说明旧授权问题，不据此断言签名错误。
