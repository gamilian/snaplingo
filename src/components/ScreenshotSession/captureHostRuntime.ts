...
import {
  copyTextToClipboard as copyCapturedTextToClipboard,
  openCaptureOcrResultWindow as openCapturedOcrResultWindow,
  openCaptureTranslationResultWindow as openCapturedTranslationResultWindow,
  outputCapture as outputCapturedSelection,
  renderCaptureOutput as renderCapturedSelection,
  runCaptureOcr as runCapturedSelectionOcr,
...
export interface CaptureHostRuntimeEffectClient {
  copyCaptureSelection: typeof copyCaptureSelection;
  saveCaptureSelection: typeof saveCaptureSelection;
  quickSaveCaptureSelection: typeof quickSaveCaptureSelection;
  printCaptureSelection: typeof printCaptureSelection;
  outputCapture: typeof outputCapturedSelection;
  runCaptureOcr: typeof runCapturedSelectionOcr;
  normalizeOcrText: typeof normalizeCapturedOcrText;
  renderCaptureOutput: typeof renderCapturedSelection;
  openCaptureOcrResultWindow: typeof openCapturedOcrResultWindow;
  openCaptureTranslationResultWindow: typeof openCapturedTranslationResultWindow;
  copyTextToClipboard: typeof copyCapturedTextToClipboard;
...
const defaultCaptureHostRuntimeEffectClient: CaptureHostRuntimeEffectClient = {
  ...
  quickSaveCaptureSelection,
  printCaptureSelection,
  outputCapture: outputCapturedSelection,
  runCaptureOcr: runCapturedSelectionOcr,
  normalizeOcrText: normalizeCapturedOcrText,
  renderCaptureOutput: renderCapturedSelection,
  openCaptureOcrResultWindow: openCapturedOcrResultWindow,
  openCaptureTranslationResultWindow: openCapturedTranslationResultWindow,
  copyTextToClipboard: copyCapturedTextToClipboard,
...
export interface RunCaptureHostPreviewRenderOptions {
  ...
  clearPreview: () => void;
  clearError: () => void;
  renderPreview?: typeof renderCapturedSelection;
...
export async function runCaptureHostPreviewRender({
  ...
  onError,
  rect,
  renderPreview = renderCapturedSelection,
  sessionId,
  setPreviewImage,
  setRendering,
...
export async function subscribeCaptureHotkeyLaunches(
  ...
  listenForHotkey: CaptureHotkeyEventListener = (eventName, handler) =>
    listenTauriEvent(eventName, handler),
  parseLaunch = parseCaptureLaunchPayload,
) {
  return listenForHotkey('hotkey-triggered', (event) => {
...
export function subscribeCaptureCancelHostRequests(
  ...
  subscribe: CaptureHostRequestSubscriber = (handler) =>
    subscribeCaptureCancelRequests(handler),
) {
...
export function subscribeCaptureCopyHostRequests(
  ...
  subscribe: CaptureHostRequestSubscriber = (handler) =>
    subscribeCaptureCopyRequests(handler),
) {
...
export interface CaptureHostWindowRevealRuntime {
  ...
  revealCaptureWindow: typeof revealCaptureWindow;
  revealCaptureWindowForSession: typeof revealCaptureWindowForSession;
}

const defaultCaptureHostWindowRevealRuntime: CaptureHostWindowRevealRuntime = {
  shouldRevealCaptureWindow,
  revealCaptureWindow,
  revealCaptureWindowForSession,
};
...
export async function revealCaptureHostWindow(
  ...
  runtime: CaptureHostWindowRevealRuntime = defaultCaptureHostWindowRevealRuntime,
) {
  if (
